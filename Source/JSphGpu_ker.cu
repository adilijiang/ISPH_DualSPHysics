...
__device__ void KerCalculateMLS3D
	...
	,double &b22,double &b23,double &b24,double &b33,double &b34,double &b44)
{
    ...
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
			const double temp=KerGetKernelWab(rr2)*volume;
...
__global__ void KerMLSBoundary3D
    ...
    if(CODE_GetTypeValue(code[p1])==1){
			...
			for(int z=zini;z<zfin;z++){
				...
				for(int y=yini;y<yfin;y++){
					...
					if(pfin) KerCalculateMLS3D (pini,pfin,posxy,posz,posmp1,CTE.massf,b11,b12,b13,b14,b22,b23,b24,b33,b34,b44);
				}
			}
...
__device__ void KerCalculateMLS2D
  (const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz
...
__global__ void KerMLSBoundary2D
  (unsigned n,int hdiv,uint4 nc,const int2 *begincell,int3 cellzero,const unsigned *dcell,
	const double2 *posxy,const double *posz,const word *code,const unsigned *idp,
	const double3 *mirrorPos,const unsigned *mirrorCell,float4 *mls)
{
    ...
    if(CODE_GetTypeValue(code[p1])==1){
			...
			for(int z=zini;z<zfin;z++){
				...
				for(int y=yini;y<yfin;y++){
					...
					if(pfin) KerCalculateMLS2D (pini,pfin,posxy,posz,posmp1,CTE.massf,b11,b12,b13,b22,b23,b33);
				}
			}
...
__global__ void KerInverseKernelCor3D(unsigned n,unsigned pinit,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,const word *code)
{
  ...
  if(p<n){
    ...
    if(CODE_GetTypeValue(code[p1])==0){
			...
			dwxcorrg[p1].x=(dwy.y*dwz.z-dwy.z*dwz.y)/det;
			dwxcorrg[p1].y=-(dwx.y*dwz.z-dwx.z*dwz.y)/det;
			dwxcorrg[p1].z=(dwx.y*dwy.z-dwx.z*dwy.y)/det;
			dwycorrg[p1].x=-(dwy.x*dwz.z-dwy.z*dwz.x)/det;
			dwycorrg[p1].y=(dwx.x*dwz.z-dwx.z*dwz.x)/det;
			dwycorrg[p1].z=-(dwx.x*dwy.z-dwx.z*dwy.x)/det;
			dwzcorrg[p1].x=(dwy.x*dwz.y-dwy.y*dwz.x)/det;
			dwzcorrg[p1].y=-(dwx.x*dwz.y-dwx.y*dwz.x)/det;
			dwzcorrg[p1].z=(dwx.x*dwy.y-dwx.y*dwy.x)/det;
    }
  }
}
...
//------------------------------------------------------------------------------
template<TpFtMode ftmode> __device__ void KerInteractionForcesBoundCode0
  (const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz,const word *code
	,const float mass,const float rhop,double3 posdp1,double3 &dwxp1,double3 &dwyp1,double3 &dwzp1,float &divrp1,unsigned &rowcount)
{
...
template<TpFtMode ftmode> __global__ void KerInteractionForcesBound
  (bool simulate2d,TpSlipCond tslipcond,unsigned n,int hdiv,uint4 nc,const int2 *begincell,int3 cellzero,const unsigned *dcell,
	...
	double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,float *divr,const double3 *mirrorPos,const unsigned *mirrorCell,const float4 *mls,unsigned *row)
{
  unsigned p1=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of particle.
  if(p1<n){
    ...
    if(CODE_GetTypeValue(code[p1])==0){
      ...
      KerGetInteractionCells(dcell[p1],hdiv,nc,cellzero,cxini,cxfin,yini,yfin,zini,zfin);
			...
			for(int fluid=0;fluid<=1;fluid++){
				for(int z=zini;z<zfin;z++){
					int zmod=(nc.w)*z+(nc.w*nc.z+1)*fluid;
					for(int y=yini;y<yfin;y++){
						int ymod=zmod+nc.x*y;
						unsigned pini,pfin=0;
						for(int x=cxini;x<cxfin;x++){
						...
						if(pfin)KerInteractionForcesBoundCode0<ftmode> (pini,pfin,posxy,posz,code,mass,CTE.rhopzero,posdp1,dwxp1,dwyp1,dwzp1,divrp1,rowCount);
					}
				}
			}
			...
			for(int fluid=0;fluid<=1;fluid++){
				for(int z=zini;z<zfin;z++){
					...
					for(int y=yini;y<yfin;y++){
						...
						if(pfin) KerCalculateMirrorVel (fluid,pini,pfin,posxy,posz,mass,CTE.rhopzero,velrhop,posmp1,Sum,divrp1,mlsp1,rowCount);
					}
				}
			}
...
template<TpFtMode ftmode> __device__ void KerInteractionForcesFluidVisc
  ...
  ,double3 posdp1,float3 velp1,float pressp1,float3 &acep1,float &divrp1,double3 &dwxp1,double3 &dwyp1,double3 &dwzp1,unsigned &rowCount)
{
  for(int p2=pini;p2<pfin;p2++){
    float drx,dry,drz;
    KerGetParticlesDr(p2,posxy,posz,posdp1,drx,dry,drz);
  ...
  for(int p2=pini;p2<pfin;p2++){
    ...
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
     ...
     /* if(USE_FLOATING){
      ...
      const float temp_x=float(frx*dwxcorrg[p1].x+fry*dwxcorrg[p1].y+frz*dwxcorrg[p1].z);
      const float temp_y=float(frx*dwycorrg[p1].x+fry*dwycorrg[p1].y+frz*dwycorrg[p1].z);
		  const float temp_z=float(frx*dwzcorrg[p1].x+fry*dwzcorrg[p1].y+frz*dwzcorrg[p1].z);
...
template<TpFtMode ftmode> __global__ void KerInteractionForcesFluid
  ...
  ,const float *ftomassp,const double2 *posxy,const double *posz,const float4 *velrhop,const word *code,const unsigned *idp
  ,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,float3 *ace,float *divr,unsigned *row,const unsigned matOrder)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
    unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
...
//==============================================================================
template<TpFtMode ftmode> void Interaction_ForcesT_KerInfo(StKerInfo *kerinfo)
{
	#if CUDART_VERSION >= 6050
  {
    typedef void (*fun_ptr)(TpInter,unsigned,unsigned,int,uint4,unsigned,float,float,const int2*,int3,const unsigned*,const float*,const double2*,const double*,const float4*,const word*,const unsigned*,double3*,double3*,double3*,float3*,float*,unsigned*,const unsigned);
    fun_ptr ptr=&KerInteractionForcesFluid<ftmode>;
  ...
  {
    typedef void (*fun_ptr)(bool,TpSlipCond,unsigned,int,uint4,const int2*,int3,const unsigned*,const float*,const double2*,const double*,float4*,const word*,const unsigned*,double3*,double3*,double3*,float*,const double3*,const unsigned*,const float4*,unsigned*);
    fun_ptr ptr=&KerInteractionForcesBound<ftmode>;
    int qblocksize=0,mingridsize=0;
    cudaOccupancyMaxPotentialBlockSize(&mingridsize,&qblocksize,(void*)ptr,0,0);
    struct cudaFuncAttributes attr;
    cudaFuncGetAttributes(&attr,(void*)ptr);
...
template<TpFtMode ftmode> void Interaction_ForcesT
  ...
  ,const float *ftomassp,float3 *ace,bool simulate2d,float *divr,const double3 *mirrorPos,const unsigned *mirrorCell,float4 *mls,unsigned *row,StKerInfo *kerinfo,JBlockSizeAuto *bsauto)
{
	const unsigned npf=np-npb;
	if(kerinfo)Interaction_ForcesT_KerInfo<ftmode>(kerinfo);
  //else if(bsauto)Interaction_ForcesT_BsAuto<psimple,tker,ftmode,lamsps,tdelta,shift>(cellmode,viscob,viscof,bsbound,bsfluid,np,npb,npbok,ncells,begincell,cellmin,dcell,posxy,posz,pospress,velrhop,code,idp,ftomassp,tau,gradvel,viscdt,ar,ace,delta,tshifting,shiftpos,shiftdetect,simulate2d,bsauto);
		...
		if(tinter==1){
			if(simulate2d) KerMLSBoundary2D<<<sgridb,bsbound>>> (npbok,hdiv,nc,begincell,cellzero,dcell,posxy,posz,code,idp,mirrorPos,mirrorCell,mls);
			else KerMLSBoundary3D<<<sgridb,bsbound>>> (npbok,hdiv,nc,begincell,cellzero,dcell,posxy,posz,code,idp,mirrorPos,mirrorCell,mls);
			KerInteractionForcesBound<ftmode> <<<sgridb,bsbound>>> (simulate2d,tslipcond,npbok,hdiv,nc,begincell,cellzero,dcell,ftomassp,posxy,posz,velrhop,code,idp,dwxcorrg,dwycorrg,dwzcorrg,divr,mirrorPos,mirrorCell,mls,row);
		}
		//-Interaccion Fluid-Fluid & Fluid-Bound
		//-Interaction Fluid-Fluid & Fluid-Bound
		const unsigned matOrder=npb-npbok;

		KerInteractionForcesFluid<ftmode> <<<sgridf,bsfluid>>> (tinter,npf,npb,hdiv,nc,cellfluid,viscob,viscof,begincell,cellzero,dcell,ftomassp,posxy,posz,velrhop,code,idp,dwxcorrg,dwycorrg,dwzcorrg,ace,divr,row,matOrder);
...
void Interaction_Forces(bool floating,bool usedem,TpSlipCond tslipcond,TpCellMode cellmode
  ...
  ,const float *ftomassp,float3 *ace,bool simulate2d,float *divr,const double3 *mirrorPos,const unsigned *mirrorCell,float4 *mls,unsigned *row,StKerInfo *kerinfo,JBlockSizeAuto *bsauto)
{
  if(!floating)   Interaction_ForcesT<FTMODE_None> (tslipcond,cellmode,viscob,viscof,bsbound,bsfluid,tinter,np,npb,npbok,ncells,begincell,cellmin,dcell,posxy,posz,velrhop,code,idp,dwxcorrg,dwycorrg,dwzcorrg,ftomassp,ace,simulate2d,divr,mirrorPos,mirrorCell,mls,row,kerinfo,bsauto);
  else if(!usedem)Interaction_ForcesT<FTMODE_Sph>  (tslipcond,cellmode,viscob,viscof,bsbound,bsfluid,tinter,np,npb,npbok,ncells,begincell,cellmin,dcell,posxy,posz,velrhop,code,idp,dwxcorrg,dwycorrg,dwzcorrg,ftomassp,ace,simulate2d,divr,mirrorPos,mirrorCell,mls,row,kerinfo,bsauto);
  else            Interaction_ForcesT<FTMODE_Dem>  (tslipcond,cellmode,viscob,viscof,bsbound,bsfluid,tinter,np,npb,npbok,ncells,begincell,cellmin,dcell,posxy,posz,velrhop,code,idp,dwxcorrg,dwycorrg,dwzcorrg,ftomassp,ace,simulate2d,divr,mirrorPos,mirrorCell,mls,row,kerinfo,bsauto);
}
...
__global__ void KerRunShifting(const bool simulate2d,unsigned n,unsigned pini,double dt
  ...
  ,float4 *velrhop,const float *divr,float3 *shiftpos,const double ShiftOffset,bool maxShift,float3 *sumtensile)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle.
  if(p<n){
    ...
    float dp=CTE.dp;
	  ...
	  float3 bitang=make_float3(0,0,0);
		rshiftpos.x+=sumtensile[p1].x; rshiftpos.y+=sumtensile[p1].y; rshiftpos.z+=sumtensile[p1].z;
...
void RunShifting(const bool simulate2d,unsigned np,unsigned npb,double dt
  ...
  if(npf){
    ...
    KerRunShifting <<<sgrid,SPHBSIZE>>> (simulate2d,npf,npb,dt,shiftcoef,freesurface,coeftfs,velrhop,divr,shiftpos,ShiftOffset,maxShift,sumtensile);
  }
}
...
{
  dim3 sgrid=GetGridSize(np,SPHBSIZE);
  if(periactive)KerMoveLinBound<true>  <<<sgrid,SPHBSIZE>>> (tslip,np,ini,Double3(mvpos),Float3(mvvel),ridp,posxy,posz,dcell,velrhop,code,idpg,mirrorPos);
  else          KerMoveLinBound<false> <<<sgrid,SPHBSIZE>>> (tslip,np,ini,Double3(mvpos),Float3(mvvel),ridp,posxy,posz,dcell,velrhop,code,idpg,mirrorPos);
}
...
__device__ void KerFindMirror
  (const double2 *posxy,const double *posz,const unsigned *idp,const unsigned idpg1,double3 posdp1
	,const int irelation,double3 *mirrorPos,bool &secondPoint,int &secondIrelation,unsigned *Physrelation)
{
	if(secondPoint){
		unsigned mirrorpoint1=irelation;
		unsigned secondmirror1=Physrelation[idp[mirrorpoint1]];
		...
		mirrorPos[idpg1].x=magnitude*(abs(drz)/sqrtf(rr2))*directionx;
		mirrorPos[idpg1].y=0;
		mirrorPos[idpg1].z=magnitude*(abs(drx)/sqrtf(rr2))*directionz;
			
		//Secondpoint
		unsigned mirrorpoint2=secondIrelation;
		unsigned secondmirror2=Physrelation[idp[mirrorpoint2]];
		...
		mirrorPos[idpg1].x=posdp1.x+2*mirrorPos[idpg1].x;
		mirrorPos[idpg1].y=0;
		mirrorPos[idpg1].z=posdp1.z+2*mirrorPos[idpg1].z;
	}
	else{
...
__device__ void KerFindIrelationCalc
  (const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz
  ,const word *code,double3 posdp1,int &irelation,float &closestr,bool &secondPoint,int &secondIrelation)
{
...
__global__ void KerConnectBoundary
  (unsigned npb,const double2 *posxy,const double *posz,const word *code,const unsigned *idp,unsigned *Physrelation)
{
  unsigned p1=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of particle.
  if(p1<npb){
...
__device__ void KerRHSandLHSStorage
  (unsigned p1,const unsigned &pini,unsigned npb,const unsigned &pfin,const double2 *posxy,const double *posz
  ,const float4 *velrhop,const float3 velp1,const float massp2,const double3 posdp1
	,const double3 dwxcorrgp1,const double3 dwycorrgp1,const double3 dwzcorrgp1,double &matrixbp1,const word *code,unsigned &rowCount)
{
  for(int p2=pini;p2<pfin;p2++){
    float drx,dry,drz;
    KerGetParticlesDr(p2,posxy,posz,posdp1,drx,dry,drz);
    ...
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
			if(CODE_GetTypeValue(code[p2])==0){
				...
				if(!(p1<int(npb)&&p2<int(npb))){
					...
					const float temp_x=float(frx*dwxcorrgp1.x+fry*dwxcorrgp1.y+frz*dwxcorrgp1.z);
					const float temp_y=float(frx*dwycorrgp1.x+fry*dwycorrgp1.y+frz*dwycorrgp1.z);
					const float temp_z=float(frx*dwzcorrgp1.x+fry*dwzcorrgp1.y+frz*dwzcorrgp1.z);
...
__global__ void KerRHSandLHSStorage
  (unsigned n,unsigned pinit,unsigned npb,unsigned npbok,int hdiv,uint4 nc,unsigned cellfluid,const int2 *begincell,int3 cellzero,const unsigned *dcell
  ...
  ,double *matrixb,const unsigned *idp,const double dt,const float *divr,const word *code,const float freesurface,unsigned *row)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
    unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
    if(CODE_GetTypeValue(code[p1])==0){
        ...
        for(int z=zini;z<zfin;z++){
          ...
          for(int y=yini;y<yfin;y++){
            ...
            if(pfin){
		          KerRHSandLHSStorage (p1,pini,npb,pfin,posxy,posz,velrhop,velp1,CTE.massf,posdp1,dwxcorrgp1,dwycorrgp1,dwzcorrgp1,matrixbp1,code,rowCount);
            }
	        }
        }
        ...
        for(int z=zini;z<zfin;z++){
          ...
          for(int y=yini;y<yfin;y++){
            ...
            if(pfin){
              KerRHSandLHSStorage (p1,pini,npb,pfin,posxy,posz,velrhop,velp1,CTE.massb,posdp1,dwxcorrgp1,dwycorrgp1,dwzcorrgp1,matrixbp1,code,rowCount);
		        }
          }
        }
...
void RHSandLHSStorage(TpCellMode cellmode,const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb
  ...
  if(npf){
    ...
    KerRHSandLHSStorage <<<sgridf,bsfluid>>> (npf,npb,npb,npbok,hdiv,nc,cellfluid,begincell,cellzero,dcell,posxy,posz,velrhop,dwxcorrg,dwycorrg,dwzcorrg,matrixb,idp,dt,divr,code,freesurface,row);
    KerRHSandLHSStorage <<<sgridb,bsbound>>> (npbok,0,npb,npbok,hdiv,nc,cellfluid,begincell,cellzero,dcell,posxy,posz,velrhop,dwxcorrg,dwycorrg,dwzcorrg,matrixb,idp,dt,divr,code,freesurface,row);
  }
}

__device__ void KerStorageCode1Find
  (const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz
  ,const double3 posdp1,unsigned &rowCount)
{
  for(int p2=pini;p2<pfin;p2++){
    float drx,dry,drz;
    KerGetParticlesDr(p2,posxy,posz,posdp1,drx,dry,drz);
    float rr2=drx*drx+dry*dry+drz*drz;
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO) rowCount++;
  }
}

__global__ void KerStorageCode1
  (unsigned n,unsigned pinit,int hdiv,uint4 nc,unsigned cellfluid,const int2 *begincell,int3 cellzero
  ,const double2 *posxy,const double *posz,const unsigned *idp,const word *code,unsigned *row,const double3 *mirrorPos,const unsigned *mirrorCell)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
    unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
...
void StorageCode1(TpCellMode cellmode,const unsigned bsbound,unsigned np,unsigned npb
  ...
  //-Interaction Fluid-Fluid & Fluid-Bound
	...
	KerStorageCode1 <<<sgridb,bsbound>>> (npbok,0,hdiv,nc,cellfluid,begincell,cellzero,posxy,posz,idp,code,row,mirrorPos,mirrorCell);
}
...
///Matrix A Population
//==============================================================================
__device__ void KerMatrixACode0
  (const unsigned matOrderp1,const unsigned matOrderp2,const unsigned &pini,const unsigned &pfin,unsigned npb,unsigned npbok,const double2 *posxy,const double *posz,double3 posdp1
	,const float3 velp1,const float4 *velrhop,const double3 dwx,const double3 dwy,const double3 dwz,tfloat3 gravity,const float massp2,const float RhopZero,const word *code,const unsigned *idp,unsigned &index
	,unsigned int *col,double *matrixInd,double *matrixb,const int diag,const double3 *mirrorPos,const unsigned oi,double &divU,double &Neumann)
{
  ...
  for(int p2=pini;p2<pfin;p2++){
    ...
    float rr2=drx*drx+dry*dry+drz*drz;
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
			const unsigned oj=p2-matOrderp2;
      //-Wendland kernel.
      float frx,fry,frz;
      KerGetKernel(rr2,drx,dry,drz,frx,fry,frz);
...
__global__ void KerPopulateMatrixACode0
  ...
  ,const double2 *posxy,const double *posz,const float4 *velrhop,const double3 *dwxCorr,const double3 *dwyCorr,const double3 *dwzCorr,const float *divr,const word *code
  ,const unsigned *idp,unsigned int *row,unsigned int *col,double *matrixInd,double *matrixb,const float freesurface,const double3 *mirrorPos,const double dt,const unsigned matOrderfluid)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
    unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
    if(CODE_GetTypeValue(code[p1])==0&&CODE_GetSpecialValue(code[p1])!=CODE_PERIODIC){
			unsigned matOrder=0;
			if(pinit) matOrder=matOrderfluid;
      ...
      unsigned index=diag+1;
			double divU=0;
			double Neumann=0;
			double3 dwx=make_double3(dwxCorr[p1].x,dwxCorr[p1].y,dwxCorr[p1].z); //  dwx.x   dwx.y   dwx.z
			double3 dwy=make_double3(dwyCorr[p1].x,dwyCorr[p1].y,dwyCorr[p1].z); //  dwy.x   dwy.y   dwy.z
			double3 dwz=make_double3(dwzCorr[p1].x,dwzCorr[p1].y,dwzCorr[p1].z); //  dwz.x   dwz.y   dwz.z
        ...
        for(int fluid=0;fluid<=1;fluid++){
					for(int z=zini;z<zfin;z++){
						...
						for(int y=yini;y<yfin;y++){
							...
							if(pfin){
								KerMatrixACode0 (matOrder,fluid*matOrderfluid,pini,pfin,npb,npbok,posxy,posz,posdp1,velp1,velrhop,dwx,dwy,dwz,gravity,CTE.massf,CTE.rhopzero,code,idp,index,col,matrixInd,matrixb,diag,mirrorPos,oi,divU,Neumann);
							}
						}
					}
				}
...
void PopulateMatrix(TpCellMode cellmode,const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb,unsigned npbok,tuint3 ncells,const int2 *begincell,tuint3 cellmin
  ...
  if(npf){
    ...
    dim3 sgridb=GetGridSize(npbok,bsbound);
		const unsigned matOrder=npb-npbok;
		KerPopulateMatrixACode0 <<<sgridf,bsfluid>>> (npf,npb,npb,npbok,hdiv,nc,cellfluid,begincell,cellzero,dcell,gravity,posxy,posz,velrhop,dwxCorr,dwyCorr,dwzCorr,divr,code,idp,row,col,matrixInd,matrixb,freesurface,mirrorPos,dt,matOrder);
    KerPopulateMatrixACode0 <<<sgridb,bsbound>>> (npbok,0,npb,npbok,hdiv,nc,cellfluid,begincell,cellzero,dcell,gravity,posxy,posz,velrhop,dwxCorr,dwyCorr,dwzCorr,divr,code,idp,row,col,matrixInd,matrixb,freesurface,mirrorPos,dt,matOrder);
...
__global__ void KerFreeSurfaceMark
  (unsigned n,unsigned pinit,unsigned npb,unsigned npbok,float *divr,double *matrixInd, double *matrixb,unsigned int *row,const word *code,const double pi,const float freesurface,const float shiftoffset)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
      unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
...
void FreeSurfaceMark(const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb,unsigned npbok,float *divr
  ...
  if(npf){
    ...
    KerFreeSurfaceMark <<<sgridf,bsfluid>>> (npf,npb,npb,npbok,divr,matrixInd,matrixb,row,code,pi,freesurface,shiftoffset);
    KerFreeSurfaceMark <<<sgridb,bsbound>>> (npbok,0,npb,npbok,divr,matrixInd,matrixb,row,code,pi,freesurface,shiftoffset);
  }
}
...
__global__ void KerPressureAssignFluid
  (unsigned npf,unsigned npb,unsigned npbok,float4 *velrhop,double *press)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<npf){
    unsigned p1=p+npb;      //-Nº de particula. //-NI of particle
    velrhop[p1].w=float(press[(p1-npb)+npbok]);
  }
}
...
void PressureAssign(const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb,unsigned npbok
  ...
  if(np){
    ...
    dim3 sgridb=GetGridSize(npb,bsbound);
      ...
      KerPressureAssignFluid <<<sgridf,bsfluid>>> (npf,npb,npbok,velrhop,press);
     ...
     // bool compute=true; //-Se desactiva cuando se usa DEM y es float-float o float-bound. //-Deactivated when DEM is used and is float-float or float-bound.
      ...
      const float tensile=tensilen*powf(KerGetKernelWab(rr2)/Wab1,tensiler);
        ...
        shiftposp1.z+=massrhop*frz;
				sumtensilep1.x+=massrhop*tensile*frx;
				sumtensilep1.y+=massrhop*tensile*fry;
				sumtensilep1.z+=massrhop*tensile*frz;
  ...
  if(p<n){
    ...
    for(int z=zini;z<zfin;z++){
      ...
      for(int y=yini;y<yfin;y++){
        ...
        if(pfin){
		      KerInteractionForcesShifting2<ftmode> (false,p1,pini,pfin,viscob,ftomassp,posxy,posz,velrhop,code,CTE.massf,ftmassp1,ftp1,posdp1,velp1,tshifting,shiftposp1,Wab1,tensilen,tensiler,divrp1,sumtensilep1);
        }
	    }
    }
    ...
    if(shiftposp1.x||shiftposp1.y||shiftposp1.z||divrp1){
      ...
      divr[p1]+=divrp1;
			sumtensile[p1].x+=sumtensilep1.x; sumtensile[p1].y+=sumtensilep1.y; sumtensile[p1].z+=sumtensilep1.z;
    ...
    for(int z=zini;z<zfin;z++){
      ...
      for(int y=yini;y<yfin;y++){
        ...
        if(pfin){
		      KerInteractionForcesShifting2<ftmode> (true,p1,pini,pfin,viscof,ftomassp,posxy,posz,velrhop,code,CTE.massf,ftmassp1,ftp1,posdp1,velp1,tshifting,shiftposp1,Wab1,tensilen,tensiler,divrp1,sumtensilep1);
        }
      }
    }
  ...
  ,TpShifting tshifting,float3 *shiftpos,float *divr,const float tensilen,const float tensiler,float3 *sumtensile)
{
  ...
  if(npf){
    ...
    if(!floating)   KerInteractionForcesShifting1<FTMODE_None> <<<sgridf,bsfluid>>> (npf,npb,hdiv,nc,cellfluid,viscob,viscof,begincell,cellzero,dcell,ftomassp,posxy,posz,velrhop,code,tshifting,shiftpos,divr,tensilen,tensiler,sumtensile);
    else if(!usedem)KerInteractionForcesShifting1<FTMODE_Sph> <<<sgridf,bsfluid>>> (npf,npb,hdiv,nc,cellfluid,viscob,viscof,begincell,cellzero,dcell,ftomassp,posxy,posz,velrhop,code,tshifting,shiftpos,divr,tensilen,tensiler,sumtensile);
    else            KerInteractionForcesShifting1<FTMODE_Dem> <<<sgridf,bsfluid>>> (npf,npb,hdiv,nc,cellfluid,viscob,viscof,begincell,cellzero,dcell,ftomassp,posxy,posz,velrhop,code,tshifting,shiftpos,divr,tensilen,tensiler,sumtensile);
  }
}
