...
template<TpKernel tker,TpFtMode ftmode> __device__ void KerInteractionForcesFluidVisc
  ...
  ,double3 posdp1,double3 velp1,double pressp1,double3 &acep1,double &divrp1,double3 &dwxp1,double3 &dwyp1,double3 &dwzp1,unsigned &rowCount,const double *divr,const float boundaryfs,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall,tfloat3 gravity)
{
	const double volume=massp2/CTE.rhopzero; //Volume of particle j 
...
template<TpKernel tker,TpFtMode ftmode> __device__ void KerInteractionForcesFluidPresGrad
  ...
  ,double3 &acep1,unsigned *row,float &nearestBound,const double *divr,const float boundaryfs,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall,tfloat3 gravity)
{
...
template<TpKernel tker,TpFtMode ftmode> __global__ void KerInteractionForcesFluid
  ...
  ,const float *ftomassp,const double2 *posxy,const double *posz,const double3 *velrhop,const word *code,const unsigned *idp
  ,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,double3 *ace,double *divr,unsigned *row,const unsigned matOrder,const float boundaryfs,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall,tfloat3 gravity)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<npf){
...
template<TpKernel tker,TpFtMode ftmode> __device__ void KerViscousSchwaigerCalc
  (bool boundp2,unsigned p1,const unsigned &pini,const unsigned &pfin,const float *ftomassp,const double2 *posxy,const double *posz
	...
	,double3 &dud,double3 &dvd,double3 &dwd,double3 dwxp1,double3 dwyp1,double3 dwzp1,const double *divr,const float boundaryfs,double3 &taop1,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall,tfloat3 gravity)
{
	const double volume=massp2/CTE.rhopzero; //Volume of particle j 
...
template<TpKernel tker,TpFtMode ftmode> __global__ void KerViscousSchwaiger
  (unsigned npf,unsigned npb,int hdiv,uint4 nc,unsigned cellfluid,float viscob,float viscof,const int2 *begincell
	...
	,double3 *ace,double3 *SumFr,double *tao,const double *divr,const float boundaryfs,const float freesurface,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall,tfloat3 gravity)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<npf){
    unsigned p1=p+npb;      //-Nº de particula. //-NI of particle
...
template<TpKernel tker,TpFtMode ftmode> void Interaction_ForcesT_KerInfo(StKerInfo *kerinfo)
{
  ...
  {
    typedef void (*fun_ptr)(TpInter,unsigned,unsigned,int,uint4,unsigned,float,float,const int2*,int3,const unsigned*,const float*,const double2*,const double*,const double3*,const word*,const unsigned*,double3*,double3*,double3*,double3*,double*,unsigned*,const unsigned,const float,const double*,const double,const double,const double,tfloat3);
...
template<TpKernel tker,TpFtMode ftmode,bool schwaiger> void Interaction_ForcesT
  ...
  ,const float *ftomassp,double3 *ace,bool simulate2d,double *divr,const double3 *mirrorPos,const unsigned *mirrorCell,double4 *mls,unsigned *row
...
void Interaction_Forces(TpKernel tkernel,bool floating,bool usedem,TpSlipCond tslipcond,bool schwaiger,TpCellMode cellmode
  ...
  ,const double2 *posxy,const double *posz,double3 *velrhop,const word *code,const unsigned *idp,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg
  ,const float *ftomassp,double3 *ace,bool simulate2d,double *divr,const double3 *mirrorPos,const unsigned *mirrorCell
...
__global__ void KerRunShifting(const bool simulate2d,unsigned n,unsigned pini,double dt
  ,float shiftcoef,float freesurface,double3 *velrhop,const double *divr,double3 *shiftpos
	...
	,const double beta0,const double beta1,double3 *NormShiftDir,double3 *TangShiftDir,double3 *ShiftDist,const double3 *normal,const unsigned *nearFS)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle.
  if(p<n){
    const unsigned p1=p+pini;
		...
		rshiftpos.x+=sumtensile[Correctp1].x;
		rshiftpos.z+=sumtensile[Correctp1].z;
		double3 N=normal[Correctp1];
    double divrp1=divr[p1];
		double h=double(CTE.h);
		double dp=double(CTE.dp);   
		
		if(divrp1<freesurface||nearFS[p1]==1){
			double magN=sqrt(N.x*N.x+N.z*N.z);
			N.x=N.x/magN; N.z=N.z/magN;
			//double3 tang;
			//tang.x=-N.z;
			//tang.z=N.x;
			double3 direction=make_double3(0,0,0);
			//double temp=tang.x*rshiftpos.x+tang.z*rshiftpos.z;
			//direction.x=temp*tang.x;
			//direction.z=temp*tang.z;
 			double3 tProdx=make_double3(0,0,0);
 			double3 tPrody=make_double3(0,0,0);
 			double3 tProdz=make_double3(0,0,0);
 			...
 			direction.x=(1.0-tProdx.x)*rshiftpos.x-tProdx.y*rshiftpos.y-tProdx.z*rshiftpos.z;
 			direction.y=-tPrody.x*rshiftpos.x+(1.0-tPrody.y)*rshiftpos.y-tPrody.z*rshiftpos.z;
 			direction.z=-tProdz.x*rshiftpos.x-tProdz.y*rshiftpos.y+(1.0-tProdz.z)*rshiftpos.z;
...
void RunShifting(const unsigned bsfluid,const bool simulate2d,unsigned np,unsigned npb,double dt
  ,double shiftcoef,float freesurface,double3 *velrhop,const double *divr,double3 *shiftpos
	,const bool maxShift,double3 *sumtensile,const float shiftoffset,const double alphashift,const double betashift0,const double betashift1,double3 *NormShiftDir,double3 *TangShiftDir,double3 *ShiftDist,const double3 *normal,const unsigned *nearFS){
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetGridSize(npf,bsfluid);
    KerRunShifting <<<sgridf,bsfluid>>> (simulate2d,npf,npb,dt,shiftcoef,freesurface,velrhop,divr,shiftpos,shiftoffset,alphashift,maxShift,sumtensile,betashift0,betashift1,NormShiftDir,TangShiftDir,ShiftDist,normal,nearFS);
  }
}
...
template<bool floating> __global__ void KerComputeStepSymplecticCor
  ...
  ,word *code,double3 *velrhop,tfloat3 gravity,const unsigned *row,double2 *posxy,double *posz,const unsigned *idp,const double3 *mirrorPos
	,const bool wavegen,const double dampingpoint,const double dampinglength,const double RightWall, const double PistonPos,const double PistonVel,double2 *posxypre,double *poszpre,unsigned *dcell)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle.
  if(p<npf){
		const unsigned p1=p+npb;
    ...
    velrhop[p1].x-=(race.x-gravity.x)*dt;
    velrhop[p1].y-=(race.y-gravity.y)*dt;
    velrhop[p1].z-=(race.z-gravity.z)*dt;
    ...
    posz[p1]=poszpre[p1]+(velrhoppre[p1].z+velrhop[p1].z)*dtm;
		...
		double dy=posxy[p1].y-CTE.maprealposminy;
		double dz=posz[p1]-CTE.maprealposminz;
	...
	,const bool wavegen,const double dampingpoint,const double dampinglength,const double RightWall, const double PistonPos,const double PistonVel,double2 *posxypre,double *poszpre,unsigned *dcell)
{
  if(np){
		const unsigned npf=np-npb;
    dim3 sgrid=GetGridSize(npf,bsfluid);
    if(floating)KerComputeStepSymplecticCor<true>  <<<sgrid,bsfluid>>> (npf,npb,velrhoppre,ace,dtm,dt,rhopoutmin,rhopoutmax,code,velrhop,gravity,row,posxy,posz,idp,mirrorPos,wavegen,dampingpoint,dampinglength,RightWall,PistonPos,PistonVel,posxypre,poszpre,dcell);
    else        KerComputeStepSymplecticCor<false> <<<sgrid,bsfluid>>> (npf,npb,velrhoppre,ace,dtm,dt,rhopoutmin,rhopoutmax,code,velrhop,gravity,row,posxy,posz,idp,mirrorPos,wavegen,dampingpoint,dampinglength,RightWall,PistonPos,PistonVel,posxypre,poszpre,dcell);
  }
}
  ...
  ,double2 *posxy,double *posz,unsigned *dcell,word *code)
{
  ...
  if(npf){
    ...
    if(periactive){ const bool peri=true;
      if(floating)KerComputeStepPos2<peri,true>  <<<sgrid,bsfluid>>> (npf,npb,posxypre,poszpre,movxy,movz,posxy,posz,dcell,code);
      else        KerComputeStepPos2<peri,false> <<<sgrid,bsfluid>>> (npf,npb,posxypre,poszpre,movxy,movz,posxy,posz,dcell,code);
    }
...
template<bool floating> __global__ void KerComputeRStar(unsigned npf,unsigned npb,const double3 *velrhoppre,double dt,word *code,double2 *posxypre,double *poszpre,double2 *posxy,double *posz,unsigned *dcell)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle.
  if(p<npf){
    unsigned p1=p+npb;
    posxy[p1].x=posxypre[p1].x+velrhoppre[p1].x*dt;
		posz[p1]=poszpre[p1]+velrhoppre[p1].z*dt;
		double dx=posxy[p1].x-CTE.maprealposminx;
		double dy=posxy[p1].y-CTE.maprealposminy;
		double dz=posz[p1]-CTE.maprealposminz;
		unsigned cx=unsigned(dx/CTE.scell),cy=unsigned(dy/CTE.scell),cz=unsigned(dz/CTE.scell);
...
void ComputeRStar(const unsigned bsfluid,bool floating,unsigned npf,unsigned npb,const double3 *velrhoppre,double dt,word *code,double2 *posxypre,double *poszpre,double2 *posxy,double *posz,unsigned *dcell)
{
  if(npf){
    dim3 sgrid=GetGridSize(npf,bsfluid);
    if(floating)KerComputeRStar<true> <<<sgrid,bsfluid>>> (npf,npb,velrhoppre,dt,code,posxypre,poszpre,posxy,posz,dcell);
    else        KerComputeRStar<false><<<sgrid,bsfluid>>> (npf,npb,velrhoppre,dt,code,posxypre,poszpre,posxy,posz,dcell);
  }
}
...
///Matrix A Setup
//==============================================================================
__global__ void kerMatrixASetup(const unsigned end,const unsigned start,const unsigned matOrder,const unsigned ppedim,unsigned int *row,unsigned *nnz,unsigned *numfreesurface,const double *divr,const float freesurface){
...
template<TpKernel tker,bool schwaiger> __device__ void KerMatrixAFluidSelf
  (const unsigned p1,const unsigned matOrder,const double2 *posxy,const double *posz,double3 posdp1
	...
	,unsigned int *col,double *matrixInd,double *matrixb,const int diag,const double3 *mirrorPos,const unsigned oi,double &divU,double &Neumann,const double3 sumfr,const double *divr,const float boundaryfs,const double taop1,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall)
{
  ...
  for(int count=0;count<=5;count++){
		...
		if(interact){
			...
			if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
				...
				double dvx=velp1.x-velp2.x, dvy=velp1.y-velp2.y, dvz=velp1.z-velp2.z;
...
template<TpKernel tker,bool schwaiger> __global__ void KerPopulateMatrixAFluid
  ...
  ,const double2 *posxy,const double *posz,const double3 *velrhop,const double3 *dwxCorr,const double3 *dwyCorr,const double3 *dwzCorr,const double *divr,const word *code
...
void PopulateMatrix(TpKernel tkernel,bool schwaiger,TpCellMode cellmode,const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb,unsigned npbok,tuint3 ncells,const int2 *begincell,tuint3 cellmin
  ...
  ,unsigned int *row,unsigned int *col,const unsigned *idp,const double *divr,const word *code,const float freesurface,const double3 *mirrorPos,const unsigned *mirrorCell,const double4 *mls,const double dt,const double3 *SumFr,const double *tao,const float boundaryfs,const double *pressure,const double PistonPos,const double PistonVel,const double RightWall){
  const unsigned npf=np-npb;
  const int hdiv=(cellmode==CELLMODE_H? 2: 1);
  const uint4 nc=make_uint4(ncells.x,ncells.y,ncells.z,ncells.x*ncells.y);
  const unsigned cellfluid=nc.w*nc.z+1;
...
__global__ void KerArrangePressureTemp(const unsigned npb,const unsigned npf,const double *pressure,double *temp)
{
	unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la particula //-NI of the particle
  if(p<npf){
		const unsigned p1=p+npb;
...
__global__ void KerCorrectResultVec(const unsigned npbok,const unsigned npf,double *pressure,const double *temp)
{
	unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la particula //-NI of the particle
  if(p<npf){
		const unsigned ppe=p;
		pressure[ppe]=temp[p];
	}
}
...
void SolverResultArrange(const unsigned bsfluid,const unsigned npb,const unsigned npbok,const unsigned npf,double *pressure){
	if(npf){
		double *temp; cudaMalloc((void**)&temp,sizeof(double)*npf); cudaMemset(temp,0,sizeof(double)*npf);
    dim3 sgridf=GetGridSize(npf,bsfluid);
    KerArrangePressureTemp <<<sgridf,bsfluid>>> (npb,npf,pressure,temp);
		KerCorrectResultVec <<<sgridf,bsfluid>>> (npbok,npf,pressure,temp);
		cudaFree(temp); temp=NULL;
  }
}
...
template<TpKernel tker,TpFtMode ftmode> __device__ void KerInteractionForcesShifting2
  ...
  ,const double2 *posxy,const double *posz,double3 *velrhop,const word *code,float massp2,float ftmassp1,bool ftp1
  ,double3 posdp1,double3 velp1,TpShifting tshifting,double3 &shiftposp1,double Wab1,const double tensilen
	, const float tensiler,double &divrp1,double3 &sumtensilep1,const float freesurface,const float boundaryfs
	,double3 &dwxp1,double3 &dwyp1,double3 &dwzp1,const double PistonPos,const double PistonVel
	,const double RightWall,const tfloat3 gravity,const double *pressure,double3 &bxp1,double3 &bzp1)
{
  for(int p2=pini;p2<pfin;p2++){
    double drx,dry,drz;
    for(int count=0;count<=5;count++){
			...
			if(interact){
				...
				if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
					...
					const double Wab_wab=Wab/Wab1;
					const double tensile=tensiler*pow(Wab_wab,tensilen);
...
template<TpKernel tker,TpFtMode ftmode> __global__ void KerInteractionForcesShifting1
  ...
  ,TpShifting tshifting,double3 *shiftpos,double *divr,const float tensilen,const float tensiler,double3 *sumtensile,const float freesurface
	,const float boundaryfs,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,const double PistonPos,const double PistonVel
	,const double RightWall,const tfloat3 gravity,const double *pressure,double3 *normal)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<n){
    unsigned p1=p+pinit;      //-Nº de particula. //-NI of particle
    ...
    for(int z=zini;z<zfin;z++){
      ...
      for(int y=yini;y<yfin;y++){
        ...
        if(pfin){
		      KerInteractionForcesShifting2<tker,ftmode> (false,p1,pini,pfin,viscof,ftomassp,posxy,posz,velrhop,code,CTE.massf,ftmassp1,ftp1,posdp1,velp1,tshifting,shiftposp1,Wab1,tensilen,tensiler,divrp1,sumtensilep1,freesurface,boundaryfs,dwxp1,dwyp1,dwzp1,PistonPos,PistonVel,RightWall,gravity,pressure,bxp1,bzp1);
        }
	    }
    }
    ...
    if(shiftposp1.x||shiftposp1.y||shiftposp1.z||divrp1){
      ...
      divr[p1]+=divrp1;
		...
		const double det=1.0/(Bx.x*Bz.z-Bz.x*Bx.z+CTE.eta2);
		...
		if(det){
			...
			BC.x=bxp1.x*shiftpos[Correctp1].x+bxp1.z*shiftpos[Correctp1].z;
 			...
 			normal[Correctp1].x=-BC.x/absB;
 			normal[Correctp1].z=-BC.z/absB;
		}
  }
}
...
__device__ void KerFindFSVicinityCalc
  ...
  ,double3 posdp1,const double *divr,const float freesurface,const float boundaryfs,unsigned &nearFS)
 {
   for(int p2=pini;p2<pfin;p2++)if(!(boundp2&&divr[p2]<boundaryfs)){
     double drx,dry,drz;
		 drx=posdp1.x-posxy[p2].x;
		 dry=posdp1.y-posxy[p2].y;
		 drz=posdp1.z-posz[p2];
     double rr2=drx*drx+dry*dry+drz*drz;
     if(divr[p2]<freesurface && rr2<=4.0*CTE.h*CTE.h){
 			nearFS=1;
 			break;
     }
   }
 ...
 __global__ void KerFindFSVicinity
   (unsigned n,unsigned pinit,int hdiv,uint4 nc,unsigned cellfluid,const int2 *begincell,int3 cellzero
 	,const unsigned *dcell,const double2 *posxy,const double *posz,double *divr,const float freesurface
...
void Interaction_Shifting
  ...
  ,unsigned np,unsigned npb,unsigned npbok,tuint3 ncells,const int2 *begincell,tuint3 cellmin,const unsigned *dcell
  ,const double2 *posxy,const double *posz,double3 *velrhop,const word *code,const float *ftomassp,TpShifting tshifting,double3 *shiftpos,double *divr,const float tensilen,const float tensiler
	,double3 *sumtensile,const float freesurface,const float boundaryfs,const unsigned *idp,const double3 *mirrorPos
	,const unsigned *mirrorCell,double3 *dwxcorrg,double3 *dwycorrg,double3 *dwzcorrg,double4 *mls,unsigned *row
	,const double pistonposx,const double PistonVel,const double RightWall,const tfloat3 gravity,const double *pressure
	,double3 *normal,double3 *extraP)
{
  const unsigned npf=np-npb;
  const int hdiv=(cellmode==CELLMODE_H? 2: 1);
  const uint4 nc=make_uint4(ncells.x,ncells.y,ncells.z,ncells.x*ncells.y);
...
template<bool floating> __global__ void KerComputeShift
  (unsigned npf,unsigned npb,const double3 *shiftpos,word *code,double2 *posxy,double *posz,unsigned *dcell)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle.
  if(p<npf){
    unsigned p1=p+npb;
		posxy[p1].x+=shiftpos[p].x;
    posz[p1]+=shiftpos[p].z;
		double dx=posxy[p1].x-CTE.maprealposminx;
		double dy=posxy[p1].y-CTE.maprealposminy;
		double dz=posz[p1]-CTE.maprealposminz;
		unsigned cx=unsigned(dx/CTE.scell),cy=unsigned(dy/CTE.scell),cz=unsigned(dz/CTE.scell);
...
void ComputeShift(bool floating,const unsigned bsfluid,unsigned np,unsigned npb,const double3 *shiftpos
  ,word *code,double2 *movxy,double *movz,double2 *posxy,double *posz,unsigned *dcell)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetGridSize(npf,bsfluid);
    if(floating)KerComputeShift<true>  <<<sgridf,bsfluid>>> (npf,npb,shiftpos,code,posxy,posz,dcell);
    else        KerComputeShift<false> <<<sgridf,bsfluid>>> (npf,npb,shiftpos,code,posxy,posz,dcell);
  }
}
...
template<TpKernel tker> __device__ void KerCalcShiftVelocity
  (const bool fluid,const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz,double3 posdp1
	...
	,const double *divr,const float boundaryfs,double3 &gradvx,double3 &gradvy,double3 &gradvz,const double PistonPos,const double PistonVel,const double RightWall,const tfloat3 gravity,const double *pressure)
{
	double volumep2=massp2/RhopZero; //Volume of particle j
  for(int p2=pini;p2<pfin;p2++){
    double drx,dry,drz;
...
template<TpKernel tker> __global__ void KerFindShiftVelocity
  ...
  ,const double2 *posxy,const double *posz,double3 *velrhop,const double3 *dwxCorr,const double3 *dwyCorr,const double3 *dwzCorr,const double *divr,const word *code
...
void CorrectShiftVelocity(const bool wavegen,TpKernel tkernel,TpCellMode cellmode,const unsigned bsbound,const unsigned bsfluid,unsigned np,unsigned npb,unsigned npbok,tuint3 ncells,const int2 *begincell,tuint3 cellmin
  ...
  ,const unsigned *idp,const double *divr,const word *code,const float boundaryfs,double3 *shiftpos,double3 *shiftvel,const double dampingpoint,const double dampinglength,const double PistonPos,const double PistonVel,const double RightWall,const tfloat3 gravity,const double *pressure){
  const unsigned npf=np-npb;
  const int hdiv=(cellmode==CELLMODE_H? 2: 1);
  const uint4 nc=make_uint4(ncells.x,ncells.y,ncells.z,ncells.x*ncells.y);
  const unsigned cellfluid=nc.w*nc.z+1;
...
template<TpKernel tker> __device__ void KerShowMirrorCalc
  (const unsigned &pini,const unsigned &pfin,const double2 *posxy,const double *posz,double3 posdp1
	,const double PistonPos,const double PistonVel,const double RightWall,double3 *extraP,unsigned &index,const tfloat3 gravity,const double3 *velrhop,const double *pressure)
{
  for(int p2=pini;p2<pfin;p2++){
    double drx,dry,drz;
		for(int count=1;count<=5;count++){
			...
			if(interact){
				double rr2=drx*drx+dry*dry+drz*drz;
				if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
					extraP[index].x=posdp1.x-drx;
					extraP[index].y=posdp1.y-dry;
					extraP[index].z=posdp1.z-drz;
					index++;
				}
			}
		}
...
template<TpKernel tker> __global__ void KerShowMirror
  (unsigned np,unsigned npb,int hdiv,uint4 nc,unsigned cellfluid,const int2 *begincell,int3 cellzero,const unsigned *dcell
  ,const double2 *posxy,const double *posz,const double PistonPos,const double PistonVel,const double RightWall,double3 *extraP,const tfloat3 gravity,const double3 *velrhop,const double *pressure)
{
  unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of particle.
  if(p==0){
		unsigned index=0;
		for(int p1=int(npb);p1<int(np);p1++){
			double dist=posxy[p1].x-PistonPos;
			if(dist*dist<=CTE.fourh2){
				double3 posdp1=make_double3(posxy[p1].x,posxy[p1].y,posz[p1]);
				//-Obtiene datos basicos de particula p1.
				//-Obtiene limites de interaccion
				...
				for(int z=zini;z<zfin;z++){
					...
					for(int y=yini;y<yfin;y++){
						...
						if(pfin){
							KerShowMirrorCalc<tker> (pini,pfin,posxy,posz,posdp1,PistonPos,PistonVel,RightWall,extraP,index,gravity,velrhop,pressure);
						}
					}
				}
			}
...
void ShowMirror(TpKernel tkernel,TpCellMode cellmode,unsigned np,unsigned npb,tuint3 ncells,const int2 *begincell,tuint3 cellmin
  ...
  //-Interaction Fluid-Fluid & Fluid-Bound
  if(tkernel==KERNEL_Quintic){				const TpKernel tker=KERNEL_Quintic;
		KerShowMirror<tker> <<<1,1>>> (np,npb,hdiv,nc,cellfluid,begincell,cellzero,dcell,posxy,posz,PistonPos,PistonVel,RightWall,extraP,gravity,velrhop,pressure);
	}
	else if(tkernel==KERNEL_Wendland){  const TpKernel tker=KERNEL_Wendland;
		KerShowMirror<tker> <<<1,1>>> (np,npb,hdiv,nc,cellfluid,begincell,cellzero,dcell,posxy,posz,PistonPos,PistonVel,RightWall,extraP,gravity,velrhop,pressure);
	}
	...
	,const unsigned *col,double *m2,double *m3,double *m4)
{
 ...
 void SchwaigerTest(const unsigned ppedim,const unsigned npb,const double *a,const double2 *posxyg,const double *poszg,const unsigned *row,const unsigned *col,double *m2,double *m3,double *m4){
  ...
  (const unsigned npf,const unsigned npb,const double2 *posxy,const double *posz,double3 *vel)
{
	unsigned p=blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Nº de la partícula //-NI of the particle
  if(p<npf){
		unsigned p1=p+npb;
		vel[p1].x=posz[p1]-2.0;
		vel[p1].z=-(posxy[p1].x-2.0);
	}
}

void SquarePatchVel(const unsigned bsfluid,const unsigned np,const unsigned npb,const double2 *posxy,const double *posz,double3 *vel){
	const unsigned npf=np-npb;
	dim3 sgridf=GetGridSize(npf,bsfluid);
	KerSquarePatchVel<<<sgridf,bsfluid>>>(npf,npb,posxy,posz,vel);
}
}