...
void JSphGpu::ConfigBlockSizes(bool usezone,bool useperi){
  ...
  if(CellMode==CELLMODE_2H||CellMode==CELLMODE_H){
    ...
    cusph::Interaction_Forces(TKernel,WithFloating,UseDEM,TSlipCond,Schwaiger,CellMode,0,0,0,0,INTER_Forces,100,50,20,TUint3(0),NULL,TUint3(0),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,Simulate2D,NULL,NULL,NULL,NULL,NULL,NULL,&kerinfo,NULL);
...
void JSphGpu::PreInteraction_Forces(TpInter tinter,double dt){
  ...
  if(tinter==1){
		...
		if(Schwaiger) cudaMemset(sumFrg,0,sizeof(float3)*npf);
	}
...
void JSphGpu::RunShifting(double dt){
  ...
  cusph::RunShifting(Simulate2D,Np,Npb,dt,ShiftCoef,FreeSurface,coeftfs,Velrhopg,Divrg,dWxCorrgShiftPos,maxShift,dWzCorrgTensile,AlphaShift,BetaShift);
}
    ...
    if(nmove){
      ...
      for(unsigned c=0;c<nmove;c++){
        ...
        if(Motion->GetMov(c,ref,mvsimple,mvmatrix)){//-Movimiento simple //-Simple movement
          ...
          cusph::MoveLinBound(PeriActive,np,pini,mvsimple,mvvel,RidpMoveg,Posxyg,Poszg,Dcellg,Velrhopg,Codeg,Idpg,MirrorPosg);
        }
  ...
  if(WaveGen){
    ...
    if(WaveGen)for(unsigned c=0;c<WaveGen->GetCount();c++){
      ...
      if(WaveGen->GetMotion(c,TimeStep+MotionTimeMod,stepdt,mvsimple,mvmatrix,nparts,idbegin)){//-Movimiento simple ///Simple movement
        ...
        cusph::MoveLinBound(PeriActive,nparts,idbegin-CaseNfixed,mvsimple,mvvel,RidpMoveg,Posxyg,Poszg,Dcellg,Velrhopg,Codeg,Idpg,MirrorPosg);
      }
      else{
        mvmatrix=OrderCode(mvmatrix);
