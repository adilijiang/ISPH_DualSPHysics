 ...
 DUALSPHYSICS  Copyright (c) 2015, Jose M. Dominguez et al. (see httpdual.sphysics.orgindex.phpdevelopers). 
 ...
 DualSPHysics is free software you can redistribute it andor modify it under the terms of the GNU General Public License as published by
 the Free Software Foundation, either version 3 of the License, or (at your option) any later version. 

 DualSPHysics is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of
 MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details. 

 You should have received a copy of the GNU General Public License, along with DualSPHysics. If not, see httpwww.gnu.orglicenses. 
...
protected
  ...
  stdstring RunMode;   -Overall mode of execution (symmetry, openmp, load balancing)   Almacena modo de ejecucion (simetria,openmp,balanceo,...).
  ...
  templateclass T T TSaveArrayCpu(unsigned np,const T datasrc)const;
  word        SaveArrayCpu(unsigned np,const word        datasrc)const{ return(TSaveArrayCpuword       (np,datasrc)); }
  unsigned    SaveArrayCpu(unsigned np,const unsigned    datasrc)const{ return(TSaveArrayCpuunsigned   (np,datasrc)); }
  float       SaveArrayCpu(unsigned np,const float       datasrc)const{ return(TSaveArrayCpufloat      (np,datasrc)); }
  tfloat4     SaveArrayCpu(unsigned np,const tfloat4     datasrc)const{ return(TSaveArrayCputfloat4    (np,datasrc)); }
  double      SaveArrayCpu(unsigned np,const double      datasrc)const{ return(TSaveArrayCpudouble     (np,datasrc)); }
  tdouble3    SaveArrayCpu(unsigned np,const tdouble3    datasrc)const{ return(TSaveArrayCputdouble3   (np,datasrc)); }
  tsymatrix3f SaveArrayCpu(unsigned np,const tsymatrix3f datasrc)const{ return(TSaveArrayCputsymatrix3f(np,datasrc)); }
  templateclass T void TRestoreArrayCpu(unsigned np,T data,T datanew)const;
  void RestoreArrayCpu(unsigned np,word        data,word        datanew)const{ TRestoreArrayCpuword       (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,unsigned    data,unsigned    datanew)const{ TRestoreArrayCpuunsigned   (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,float       data,float       datanew)const{ TRestoreArrayCpufloat      (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,tfloat4     data,tfloat4     datanew)const{ TRestoreArrayCputfloat4    (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,double      data,double      datanew)const{ TRestoreArrayCpudouble     (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,tdouble3    data,tdouble3    datanew)const{ TRestoreArrayCputdouble3   (np,data,datanew); }
  void RestoreArrayCpu(unsigned np,tsymatrix3f data,tsymatrix3f datanew)const{ TRestoreArrayCputsymatrix3f(np,data,datanew); }
  ...
  void ConfigRunMode(const JCfgRun cfg,stdstring preinfo=);
  ...
  templatebool psimple,TpFtMode ftmode void InteractionForcesBound
  ...
  templatebool psimple,TpFtMode ftmode void InteractionForcesFluid
  ...
  templatebool psimple void InteractionForcesDEM
  ...
  templatebool psimple,TpFtMode ftmode void Interaction_ForcesT
  ...
  templatebool shift void ComputeSymplecticPreT(double dt);
  void ComputeSymplecticPre(double dt);
  templatebool shift void ComputeSymplecticCorrT(double dt);
  ...
  void GetTimersInfo(stdstring &hinfo,stdstring &dinfo)const;
  ...
  stdstring TimerGetName(unsigned ct)const{ return(TmcGetName((CsTypeTimerCPU)ct)); }
  stdstring TimerToText(unsigned ct)const{ return(JSphTimerToText(TimerGetName(ct),TimerGetValue(ct))); }
  ...
  void MatrixStorage(bool psimple,unsigned n,unsigned pinit,tint4 nc,int hdiv,unsigned cellinitial,const unsigned beginendcell,tint3 cellzero,const unsigned dcell,const tdouble3 pos,const tfloat3 pspos,
	  float divr,stdvectorint &row,const unsigned porder,const unsigned idpc,const word code,const unsigned ppedim,const float freesurface)const;
  void MatrixASetup(const unsigned ppedim,unsigned &nnz,stdvectorint &row)const;
  void PopulateMatrixB(bool psimple,unsigned n,unsigned pinit,tint4 nc,int hdiv,unsigned cellinitial,const unsigned beginendcell,tint3 cellzero,
	  const unsigned dcell,const tdouble3 pos,const tfloat3 pspos,const tfloat4 velrhop,tdouble3 dwxcorr,tdouble3 dwzcorr,stdvectordouble &matrixb,const unsigned porder,
  ...
  void PopulateMatrixAFluid(bool psimple,unsigned n,unsigned pinit,tint4 nc,int hdiv,unsigned cellinitial,const unsigned beginendcell,tint3 cellzero,
	  const unsigned dcell,const tdouble3 pos,const tfloat3 pspos,const tfloat4 velrhop,float divr,stdvectordouble &matrixInd,stdvectorint &row,stdvectorint &col,
    const unsigned porder,const unsigned irelation,stdvectordouble &matrixb,const unsigned idpc,const word code,const unsigned ppedim,const float freesurface,tfloat3 gravity,const double rhoZero)const;
   void PopulateMatrixABoundary(bool psimple,unsigned n,unsigned pinit,tint4 nc,int hdiv,unsigned cellinitial,const unsigned beginendcell,tint3 cellzero,
	  const unsigned dcell,const tdouble3 pos,const tfloat3 pspos,const tfloat4 velrhop,float divr,stdvectordouble &matrixInd,stdvectorint &row,stdvectorint &col,
    const unsigned porder,const unsigned irelation,stdvectordouble &matrixb,const unsigned idpc,const word code,const unsigned ppedim,const float freesurface,tfloat3 gravity,const double rhoZero)const;
    ...
    stdvectordouble &matrixx,const word code,const unsigned npb,float divr,tfloat3 gravity)const;
  void FreeSurfaceMark(unsigned n,unsigned pinit,float divr,stdvectordouble &matrixInd,stdvectordouble &matrixb,stdvectorint &row,const unsigned porder,const unsigned idpc,const word code,const unsigned ppedim)const;

  void solveVienna(TpPrecond tprecond,TpAMGInter tamginter,double tolerance,int iterations,float strongconnection,float jacobiweight, int presmooth,int postsmooth,int coarsecutoff,stdvectordouble &matrixa,stdvectordouble &matrixb,stdvectordouble &matrixx,stdvectorint &row,stdvectorint &col,const unsigned ppedim,const unsigned nnz);
  ...
  templateTpFtMode ftmode void InteractionForcesShifting
