...
void RunShifting(const bool simulate2d,unsigned np,unsigned npb,double dt
  ,double shiftcoef,float freesurface,float4 *velrhop,const float *divr,float3 *shiftpos
	,const bool maxShift,float3 *sumtensile,const float shiftoffset,const double alphashift,const double beta0,const double beta1,const float3 *normal,const unsigned *nearFS);
...
void Interaction_Shifting(TpKernel tkernel,TpSlipCond tslipcond,bool simulate2d,bool floating,bool usedem,TpCellMode cellmode
  ...
  ,TpShifting tshifting,float3 *shiftpos,float *divr,const float tensilen,const float tensiler,float3 *sumtensile
	...
	,float3 *dwxcorrg,float3 *dwycorrg,float3 *dwzcorrg,float4 *mls,unsigned *row,const double PistonPos,float3 *normal);
